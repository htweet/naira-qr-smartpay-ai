...
import { Lock, Unlock, AlertTriangle, RefreshCcw, Shield, Eye } from "lucide-react";
...
const EscrowManager = ({ merchantId }: EscrowManagerProps) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogTrigger asChild>
            <Button><Lock className="mr-2 h-4 w-4" />Create Escrow</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Create Escrow</DialogTitle>
              <DialogDescription>Hold funds securely until conditions are met</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>Amount</Label>
                  <Input type="number" placeholder="0.00" value={newEscrow.amount || ""} onChange={(e) => setNewEscrow((prev) => ({ ...prev, amount: parseFloat(e.target.value) || 0 }))} />
                </div>
                <div className="space-y-2">
                  <Label>Currency</Label>
                  <Select value={newEscrow.currency || "NGN"} onValueChange={(value) => setNewEscrow((prev) => ({ ...prev, currency: value }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {supportedCurrencies.map((c) => (<SelectItem key={c.code} value={c.code}>{c.symbol} {c.code}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <Input placeholder="Customer identifier" value={newEscrow.customer_id || ""} onChange={(e) => setNewEscrow((prev) => ({ ...prev, customer_id: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Release Conditions</Label>
                <Textarea placeholder="Describe the conditions for releasing funds..." value={newEscrow.release_conditions} onChange={(e) => setNewEscrow((prev) => ({ ...prev, release_conditions: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Release Date (Optional)</Label>
                <Input type="date" value={newEscrow.release_date || ""} onChange={(e) => setNewEscrow((prev) => ({ ...prev, release_date: e.target.value }))} />
              </div>
      ...
      {/* Detail View */}
      <Dialog open={!!selectedEscrow} onOpenChange={(open) => { if (!open) { setSelectedEscrow(null); setDisputeReason(""); } }}>
        <DialogContent className="max-w-lg">
          {selectedEscrow && (
            <>
              <DialogHeader>
                <div className="flex items-center justify-between">
                  <DialogTitle>Escrow #{selectedEscrow.id.slice(0, 8)}</DialogTitle>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-muted/50 rounded-lg">
                    ...
                    <p className="text-xl font-bold">{formatCurrency(selectedEscrow.amount, selectedEscrow.currency)}</p>
                  </div>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-xs text-muted-foreground">Currency</p>
                    <p className="text-lg font-medium">{selectedEscrow.currency}</p>
                  </div>
                </div>
                {selectedEscrow.release_conditions && (
                  <div className="p-3 border rounded-lg">
                    <p className="text-sm font-medium">Release Conditions</p>
                    <p className="text-sm text-muted-foreground">{selectedEscrow.release_conditions}</p>
                  </div>
                )}
                {selectedEscrow.release_date && (
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-xs text-muted-foreground">Scheduled Release</p>
                    <p className="text-sm font-medium">{format(new Date(selectedEscrow.release_date), "PPP")}</p>
                  </div>
                )}
                {selectedEscrow.released_at && (
                  <div className="p-3 bg-emerald-50 border-emerald-200 border rounded-lg">
                    <p className="text-sm text-emerald-700">{selectedEscrow.status === "refunded" ? "Refunded" : "Released"} on {format(new Date(selectedEscrow.released_at), "PPP")}</p>
                  </div>
                )}
                {selectedEscrow.dispute_reason && (
                  <div className="p-3 bg-red-50 border-red-200 border rounded-lg">
                    ...
                    <p className="text-sm text-red-800">{selectedEscrow.dispute_reason}</p>
                  </div>
                )}
                <Separator />
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Created {formatDistanceToNow(new Date(escrow.created_at), { addSuffix: true })}</CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-2xl font-bold">{formatCurrency(escrow.amount, escrow.currency)}</p>
                    {escrow.release_conditions && <p className="text-sm text-muted-foreground line-clamp-1">Conditions: {escrow.release_conditions}</p>}
                  </div>
