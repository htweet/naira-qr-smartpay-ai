...
import { Plus, Split, Users, Clock, CheckCircle, XCircle, Eye } from "lucide-react";
...
const SplitPaymentsManager = ({ merchantId }: SplitPaymentsManagerProps) => {
  ...
  const handleAddRecipient = () => {
    setRecipients((prev) => [...prev, { recipient_account: "", recipient_bank: "", percentage: 0 }]);
  };
  ...
  const handleRecipientChange = (index: number, field: keyof SplitRecipient, value: string | number) => {
    setRecipients((prev) => {
      const updated = [...prev];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="mr-2 h-4 w-4" />Create Split</Button>
          </DialogTrigger>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Create Split Payment</DialogTitle>
              <DialogDescription>Define how to split the payment among recipients</DialogDescription>
            </DialogHeader>
                ...
                {recipients.map((recipient, index) => (
                  <div key={index} className="grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-4">
                      <Input placeholder="Account No." value={recipient.recipient_account || ""} onChange={(e) => handleRecipientChange(index, "recipient_account", e.target.value)} />
                    </div>
                    <div className="col-span-4">
                      <Input placeholder="Bank Name" value={recipient.recipient_bank || ""} onChange={(e) => handleRecipientChange(index, "recipient_bank", e.target.value)} />
                    </div>
                    <div className="col-span-3">
                      <div className="relative">
                        <Input type="number" placeholder="%" value={recipient.percentage || ""} onChange={(e) => handleRecipientChange(index, "percentage", parseFloat(e.target.value) || 0)} />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">%</span>
                      </div>
                    </div>
                    <div className="col-span-1">
                      {recipients.length > 1 && <Button variant="ghost" size="icon" onClick={() => handleRemoveRecipient(index)}><XCircle className="h-4 w-4" /></Button>}
                    </div>
                  </div>
                ))}
        ...
        <DialogContent className="max-w-lg">
          {selectedSplit && (
            <>
              <DialogHeader>
                <div className="flex items-center justify-between">
                  <DialogTitle>Split #{selectedSplit.id.slice(0, 8)}</DialogTitle>
              ...
              <div className="space-y-4">
                <div className="p-3 bg-muted/50 rounded-lg">
                  ...
                  <p className="text-xl font-bold">{formatCurrency(selectedSplit.amount, "NGN")}</p>
                  {selectedSplit.percentage && <p className="text-sm text-muted-foreground">{selectedSplit.percentage}% of total</p>}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  {selectedSplit.recipient_bank && (
                    <div className="p-3 bg-muted/50 rounded-lg">
                      <p className="text-xs text-muted-foreground">Bank</p>
                      <p className="text-sm font-medium">{selectedSplit.recipient_bank}</p>
                    </div>
                  )}
                  {selectedSplit.recipient_account && (
                    <div className="p-3 bg-muted/50 rounded-lg">
                      <p className="text-xs text-muted-foreground">Account</p>
                      <p className="text-sm font-mono">{selectedSplit.recipient_account}</p>
                    </div>
                  )}
                </div>
                {selectedSplit.processed_at && (
                  <div className="p-3 bg-emerald-50 border-emerald-200 border rounded-lg">
                    <p className="text-sm text-emerald-700">Processed on {format(new Date(selectedSplit.processed_at), "PPP 'at' p")}</p>
                  </div>
                )}
                <Separator />
                {selectedSplit.status === "pending" && (
                  <Button onClick={() => { processSplit(selectedSplit.id); setSelectedSplit(null); }} className="w-full">Process Now</Button>
                )}
              </div>
            </>
          )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-blue-50 border-blue-200"><CardContent className="p-4"><div className="flex items-center gap-2"><Users className="h-5 w-5 text-blue-600" /><div><p className="text-sm text-blue-600">Total Splits</p><p className="text-2xl font-bold text-blue-900">{stats.total}</p></div></div></CardContent></Card>
      </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Created {formatDistanceToNow(new Date(split.created_at), { addSuffix: true })}</CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-2xl font-bold">{formatCurrency(split.amount, "NGN")}</p>
                    <p className="text-sm text-muted-foreground">{split.recipient_bank ? `To: ${split.recipient_bank} - ${split.recipient_account}` : "Multiple recipients"}</p>
                  </div>
                  <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                    <Button variant="ghost" size="sm" onClick={() => setSelectedSplit(split)}><Eye className="h-4 w-4" /></Button>
                    {split.status === "pending" && <Button size="sm" onClick={() => processSplit(split.id)}>Process Now</Button>}
                  </div>
                </div>
              </CardContent>
            </Card>
